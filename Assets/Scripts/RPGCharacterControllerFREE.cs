...
public class RPGCharacterControllerFREE : MonoBehaviour 
{
	...
	void Update()
	{
		...
		if(animator)
		{
			...
			if (Physics.Raycast (pos, transform.forward, out hit, 1.6f)) {
				objetoEnFrente = true;
				GameObject obj = hit.collider.gameObject;
				//if (obj.tag == "chest") {
					//StartCoroutine (_OpenChest ());
				//}
				if (obj.tag == "TeleportableObject" || obj.tag == "key") {
	...
	void FixedUpdate()
	{
		...
		if (Input.GetButtonDown ("CogerObjeto") && cargandoObjeto && !objetoEnFrente) {
	...
	void OnCollisionEnter(Collision other){
		if (other.gameObject.tag == "KillZone" && other.rigidbody.velocity.sqrMagnitude > 0.5f && !isDead) {
			StartCoroutine (_Death());
		}
	}
	#endregion
	...
	void Jumping()
	{
		...
		{    
			...
			if(canDoubleJump && doublejumping && Input.GetButtonDown("Jump") && !doublejumped && isFalling && !isDead)
			{
	...
	/*IEnumerator _OpenChest(){
		if (Input.GetButtonDown ("CastL")) {
			var cofre = GameObject.FindGameObjectWithTag ("chest");
			yield return new WaitForSeconds(.5f);
			cofre.GetComponent<OpenChest> ().Open ();
			GameObject objetoCofre;
			//if (cofre.GetComponent<OpenChest> ().objeto.tag != "key") {
				objetoCofre = (GameObject)Instantiate (cofre.GetComponent<OpenChest> ().objeto, objectPosition.position, cofre.GetComponent<OpenChest> ().objeto.transform.rotation);
			//} else {
			//	objetoCofre = (GameObject)Instantiate (cofre.GetComponent<OpenChest> ().objeto, mano.position, mano.rotation);
			//}
			objeto = objetoCofre;
			if (!cargandoObjeto) {
				cargandoObjeto = true;
